...
| `MERGE` | 新旧 Fact 可以合并成更完整的事实 | 生成新的合并 Fact，并通过 `merge.source_ids` 和 `merge.target_id` 保存双向链路 |

凡是被判定为 `REPLACE` 或 `MERGE` 的记忆，都通过 `linked` 列表保存关系参与者 ID。

### 1.1 新增字段

Fact 正文保持不可变，只新增以下三个关系字段：
...
| `merge` | `object` | `source_ids` 表示当前合并 Fact 的直接来源；`target_id` 表示当前 Fact 被合并进了哪条 Fact |

`merge` 不能设计成布尔值。布尔值只能说明“发生过合并”，无法表达来源、合并结果以及后续再次合并的链路。
...
flowchart TD
    ...
    L --> N["写入 merge.source_ids 与 merge.target_id"]
    N --> M
    M --> O["反向映射 ID、校验并事务落库"]
```
...
- 默认优先返回没有被 `replaced_by` 或 `merge.target_id` 指向其他记忆的旧 Fact；
...
不修改旧 Fact
不设置 replaced_by
merge 保持 {"source_ids": [], "target_id": null}
不增加 linked
第一阶段写入的新 Fact 保持有效
```
...
`O1`、`O2` 和 `N1` 的正文均不覆盖。来源 Fact 通过 `merge.target_id` 指向合并结果，`M1` 通过 `merge.source_ids` 保存直接来源。为避免递归合并，`M1` 不在当前批次中再次进入第二阶段。

如果 `M1` 后续又与其他 Fact 合并为 `M2`，则保留 `M1.merge.source_ids`，同时设置 `M1.merge.target_id = M2`。因此一条 Fact 可以既是上一次合并的结果，又是下一次合并的来源。
...
6. 同一旧 Fact 在一个批次内不能同时被多个新 Fact 设置不同的 `replaced_by` 或 `merge.target_id`；
7. 更新前重新读取旧 Fact，不能覆盖已经存在的 `replaced_by` 或 `merge.target_id`；
8. `linked` 使用集合去重，不能包含自身 ID；
9. `merge.source_ids` 使用集合去重，不能包含合并 Fact 自身 ID；
10. 同一 Fact 不能同时设置 `replaced_by` 和 `merge.target_id`，避免出现两个有效后继；
11. 沿 `replaced_by` 或 `merge.target_id` 查找当前有效 Fact 时必须检测环路；
12. 合并 Fact、关系字段和 `linked` 列表在同一事务中写入。
